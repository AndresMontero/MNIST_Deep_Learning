...
class ConvNetSiam_noWS_noDr_noBN(nn.Module):
    # Define The Conv Network
    def __init__(self, hidden_layers):
        super(ConvNetSiam_noWS_noDr_noBN, self).__init__()
        ...
        self.fc1a = nn.Linear(4 * 4 * 64, hidden_layers)
        self.fc1b = nn.Linear(4 * 4 * 64, hidden_layers)

        # hidden_layers input features, 10 output features
        self.fc2a = nn.Linear(hidden_layers, 10)
        self.fc2b = nn.Linear(hidden_layers, 10)
    ...
    def forward(self, x):
        ...
        x1 = F.relu(self.fc1a(x1.view(-1, 4 * 4 * 64)))
        x2 = F.relu(self.fc1b(x2.view(-1, 4 * 4 * 64)))
